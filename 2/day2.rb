...
def validate_password (input)
  ...
  count = input[:password].chars.select { |x| x == input[:character] }.length
  range.include? count
end
...
PARSED_DATA = DATA.map { |d| parse d }

puzzle1_answer = 0
PARSED_DATA.each do |e|
  puzzle1_answer += 1 if validate_password(e)
end

puts "AoC 2020 2-1: #{puzzle1_answer}"
